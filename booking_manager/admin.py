...
class BookingAdmin(admin.ModelAdmin):
    ...
    def view_booking_and_revenue_charts(self, request, queryset):
        # Biểu đồ số lượng đặt phòng theo giờ trong ngày
        booking_count_data = {f"{hour}:00": 0 for hour in range(24)}  # Khởi tạo các khóa từ "0:00" đến "23:00"
        ...
        # Tạo biểu đồ số lượng đặt phòng theo giờ
        fig1, ax1 = plt.subplots(figsize=(10, 6))  # Biểu đồ số lượng đặt phòng
 ...
 '18:00-19:00', '19:00-20:00', '20:00-21:00', '21:00-22:00', '22:00-23:00', '23:00-0:00']
        ...
        # Biểu đồ 1: Số lượng đặt phòng theo giờ
        ...
        revenue_data = {f"{month}": 0 for month in range(1, 13)}  # Tạo khóa từ tháng 1 đến tháng 12
        for booking in bookings:
            booking_month = booking.checkin_date.month  # Có thể thay bằng checkout_date nếu cần
            revenue_data[f"{booking_month}"] += booking.get_amount()
        ...
        buf2.close()
        plt.close(fig2)  # Đảm bảo đóng biểu đồ sau khi lưu
    ...
    def export_booking_data_to_excel(self, request, queryset):
        ...
        for booking in bookings:
            # Sử dụng username của user thay vì đối tượng user
            ...
            ws.append([
                ...
                booking.get_amount(),  # Sử dụng phương thức get_amount()
                booking.status,
                booking.homestay.name,  # Lấy tên của Homestay thay vì đối tượng Homestay
                user_value,  # Sử dụng username của User
            ])
